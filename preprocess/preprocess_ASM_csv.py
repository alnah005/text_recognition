...
def preprocess_ASM_csv(print_letters=False):
    ...
    for i in range(len(clas)):
        ...
        for fr in frames:
            ...
            data_mini["boston_public_library"] = [clas1['subject_data_dict'][str(clas1["subject_ids"])]['#repository_id']+'--'+clas1['subject_data_dict'][str(clas1["subject_ids"])]['image'+str(fr+1)] for i in d_loop] 
            try:
                width_height = str(clas1['metadata_dict']['subject_dimensions'][fr]['clientWidth'])+"_"+str(clas1['metadata_dict']['subject_dimensions'][fr]['clientHeight'])
            except:
                width_height = None
            data_mini["client_width_height"] = [width_height for i in d_loop] 
            try:
                width_height = str(clas1['metadata_dict']['subject_dimensions'][fr]['naturalWidth'])+"_"+str(clas1['metadata_dict']['subject_dimensions'][fr]['naturalHeight'])
            except:
                width_height = None
            data_mini["natural_width_height"] = [width_height for i in d_loop]
